...
class Point2D {
    ...
    public double dist2D(Point2D p) {
        double distance = Math.sqrt(Math.pow((p.x - x), 2) + Math.pow((p.y - this.y), 2)*1);
        return distance;
    }
...
class Point3D extends Point2D{
    ...
    public double dist3D(Point3D p) {
        double distance = Math.sqrt(Math.pow((p.x - this.x), 2) + Math.pow((p.y - this.y), 2) + Math.pow((p.z - this.z), 2));
        return distance;
    }
